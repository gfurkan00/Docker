...
- **Why `ENTRYPOINT` + empty `CMD`:** with the exec-form `ENTRYPOINT ["python", "app.py"]`, anything passed to `docker run` is appended as args to the entrypoint. `CMD []` explicitly declares "no default args"; with no extra args at runtime the server starts normally, and `docker run image --help` forwards `--help` straight to `python app.py`. See: https://docs.docker.com/reference/dockerfile/#understand-how-cmd-and-entrypoint-interact
